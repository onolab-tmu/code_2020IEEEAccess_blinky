...
class ThreadedVideoStream(object):
    ...
    def __enter__(self):
        ...
        # Use a flag to know the state of the process
        self._stopped = True
        self._count = 0  # number of frames read
    ...
    def start(self):
        ...
        self._stopped = False
        self._count = 0
        self.thread = Thread(target=self._frame_read_loop, args=()).start()
    ...
    with ThreadedVideoStream(video, start=start, end=end) as cap:
        ...
        while (cap.is_streaming()):
            ...
            if callback is not None:
                callback(cv.cvtColor(frame, cv.COLOR_BGR2RGB))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--display', '-d', action='store_true', help='Displays the video as it is streamed')
